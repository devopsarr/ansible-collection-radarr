...
DOCUMENTATION = r'''
---
module: radarr_quality

short_description: Manages Radarr quality.

version_added: "1.0.0"

description: Manages Radarr quality.
...
def update_quality(want, result):
    ...
    if not module.check_mode:
        try:
            ...
            module.fail_json('Error updating quality: %s\n body: %s' % (to_native(e.reason), to_native(e.body)), **result)
        except Exception as e:
            module.fail_json('Error updating quality: %s' % to_native(e), **result)
...
def run_module():
    ...
    # No delete is needed
    want = radarr.QualityDefinitionResource(
    ...
    want.quality = radarr.Quality(**result['quality'])
    if want != state:
